
export const ru = {
  // Header
  coins: 'Монеты',
  
  // Navigation
  main: 'Главная',
  cases: 'Кейсы',
  quiz: 'Викторина',
  tasks: 'Задания',
  inventory: 'Инвентарь',
  settings: 'Настройки',
  
  // Recent Wins
  recentWins: 'Последние выигрыши',
  won: 'выиграл',
  loading: 'Загрузка...',
  player: 'Игрок',
  unknownItem: 'Неизвестный предмет',
  item: 'Предмет',
  noWinsYet: 'Пока никто ничего не выиграл',
  
  // Errors
  errorLoadingWins: 'Ошибка загрузки выигрышей',
  
  // Settings
  accountSettings: 'Аккаунт',
  appSettings: 'Приложение',
  support: 'Поддержка',
  premiumSubscription: 'Премиум подписка',
  unlockAllFeatures: 'Откройте все возможности',
  getPremium: 'Получить премиум',
  connectSteam: 'Подключить Steam',
  setupSkinWithdrawal: 'Настройте вывод скинов',
  promoCodes: 'Промокоды',
  activateBonuses: 'Активировать бонусы',
  appLanguage: 'Язык приложения',
  sounds: 'Звуки',
  soundEffects: 'Звуковые эффекты',
  vibration: 'Вибрация',
  tactileFeedback: 'Тактильные отклики',
  privateProfile: 'Приватный профиль',
  hideStats: 'Скрыть статистику',
  faq: 'Часто задаваемые вопросы',
  findAnswers: 'Найдите ответы на вопросы',
  
  // Toasts
  languageChanged: 'Язык изменен',
  languageSettingsSaved: 'Настройки языка успешно сохранены',
  error: 'Ошибка',
  failedToChangeLanguage: 'Не удалось изменить язык',
  settingsSaved: 'Настройки сохранены',
  changesApplied: 'Изменения успешно применены',
  failedToSaveSettings: 'Не удалось сохранить настройки',
  
  // Common
  accountManagement: 'Управление аккаунтом и предпочтениями',
};
